...
export const andonTableColumnsByYear: ColumnDef<AndonByYearDataDto, unknown>[] = [
  ...
  {
    accessorKey: "year_1",
    header: ({ table }) => <strong>{table.getRowModel().rows[0]?.original.year_numbers?.[0] || 'Year 1'}</strong>,
    cell: ({ row }) => formatCellValue(row.original.title, row.getValue("year_1")),
  },
  {
    accessorKey: "year_2",
    header: ({ table }) => <strong>{table.getRowModel().rows[0]?.original.year_numbers?.[1] || 'Year 2'}</strong>,
    cell: ({ row }) => formatCellValue(row.original.title, row.getValue("year_2")),
  },
];
